...
public class MedicoController {
    ...
    @GetMapping
    public ResponseEntity<Page<DatosListadoMedico>> listadoMedico(@PageableDefault(size = 2,page = 0,sort = "nombre") Pageable pagintaion){
//        return medicoRepository.findAll(pagintaion).map(DatosListadoMedico::new);
        return ResponseEntity.ok(medicoRepository.findByActivoTrue(pagintaion).map(DatosListadoMedico::new));
    }
...
//    }
    ...
    @GetMapping("/{id}")
    public ResponseEntity<DatosRespuestaMedico> retormarMedico(@PathVariable Long id) {
